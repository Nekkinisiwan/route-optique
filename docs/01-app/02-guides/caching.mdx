...
Next.js wraps (with custom logic and React `cache` function) the [`fetch` API](#fetch) requests that use the `GET` or `HEAD` methods to automatically **memoize** those requests that have the same URL and options. This means you can call a fetch function for the same data in multiple places in a React component tree while only executing it once.
...
> **Good to know**:
>
> - Request memoization of `fetch` requests using the `GET` or `HEAD` methods is achieved with React `cache` function and custom logic - as such it is a Next.js feature. It's included here to show how it interacts with the other caching mechanisms.
>   Note that in React itself, all memoization with React `cache` (for all types of requests, including `fetch` requests using the `GET` or `HEAD` methods) is opt-in and has to be manually implemented.
> - Memoization only applies to the `GET` or `HEAD` method in `fetch` requests.
> - Memoization only applies to the React Component tree. This means:
>   - It applies to `fetch` requests in `generateMetadata`, `generateStaticParams`, Layouts, Pages, and other Server Components.
>   - It doesn't apply to `fetch` requests in Route Handlers as they are not a part of the React component tree.
> - For cases where `fetch` is not suitable (e.g., some database clients, CMS clients, or GraphQL clients), you can use the [React `cache` function](#opting-into-memoization-for-all-requests) to memoize functions.
...
### Opting Out Of Default Memoization
...
### Opting Into Memoization For All Requests

As mentioned above, default memoization in Next.js is built upon the React `cache` function that allows you to memoize the return value of a function, allowing you to call the same function multiple times while only executing it once. `fetch` requests using the `GET` or `HEAD` methods are automatically memoized, so you do not need to wrap it in React `cache`.
However, for other `fetch` methods, or when using data-fetching libraries (such as some database, CMS, or GraphQL clients) that don't inherently memoize requests, you can use `cache` to manually memoize data requests.
