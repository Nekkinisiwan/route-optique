...
If images fail to load on authenticated routes, ensure `_next/image` is excluded from the matcher:
...
When middleware runs on image optimization requests, it can interfere with internal request forwarding. For protected images, create a secure API route with domain allowlisting to prevent SSRF attacks.

### Middleware not running on routes

Check your matcher pattern includes the intended paths:

```tsx filename="middleware.ts"
export const config = {
  // Matches /dashboard and sub-routes
...
Use logging to debug which routes middleware processes:
...
Avoid database calls in middleware. Use cookie-based optimistic checks:
...
Perform full authentication in page components or API routes.

### Edge Runtime compatibility

Use Edge Runtime compatible libraries:
...
Move complex logic to API routes for Node.js runtime access.
